...
class Element :
    ...
    def toxml(self) :
        ...
        for attr in self.attributes :
            buffer += ' %s="%s"' % (attr, self.attribute_values[attr])
        if self.childNodes :
            buffer += ">"
            for child in self.childNodes :
                buffer += child.toxml()
...
# Postprocessors munge source text before we start doing anything too
# complicated.
#
# Each postprocessor implements a "run" method that takes a pointer to
...
class ImagePattern (BasePattern):
    ...
    def handleMatch(self, m, doc) :
        ...
        el.setAttribute('src', m.group(9))
        el.setAttribute('alt', m.group(2))
        return el
...
BACKTICK_PATTERN        = SimpleTagPattern(BACKTICK_RE, 'code')
DOUBLE_BACKTICK_PATTERN = SimpleTagPattern(DOUBLE_BACKTICK_RE, 'code')
